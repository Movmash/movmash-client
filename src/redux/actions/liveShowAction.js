...
export const getLiveShowDetail = (roomCode, history) => (dispatch) => {
  ...
  axios
    .get(`http://localhost:8000/api/v1/live/get-live-show-details/${roomCode}`)
    .then((res) => {
...
export const getGenreLiveShow = (genre) => (dispatch) => {
  if (genre === "MyFamily") {
    ...
    axios
      .get("http://localhost:8000/api/v1/live/get-followings-live-show")
      .then((res) => {
    ...
    axios
      .get(`http://localhost:8000/api/v1/live/get-genre-live-show/${genre}`)
      .then((res) => {
...
export const createLiveShow = (showDetail, history) => (dispatch) => {
  dispatch({ type: LOADING_CREATE_LIVESHOW });
  axios
    .post("http://localhost:8000/api/v1/live/create-live-show", showDetail)
    .then((res) => {
